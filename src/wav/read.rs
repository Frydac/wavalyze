...
pub fn read_to_file(config: ReadConfig, buffer_pool: &mut BufferPool) -> Result<File> {
    ...
    for (ch_ix, buffer) in buffers.into_iter() {
...
// read samples into buffers with type S
fn read_to_buffers<S>(
    reader: &mut hound::WavReader<std::io::BufReader<std::fs::File>>,
    config: ReadConfig,
) -> Result<HashMap<ChIx, Buffer<S>>>
where
    S: crate::audio::buffer2::Sample + hound::Sample,
{
    ...
    // Seek to the start position if we need to skip samples
    ...
    // channel indices we want to read
